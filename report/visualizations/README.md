...
### 機能分類ジオラマページの構成（v3: ダッシュボード追加）

各ターゲットのジオラマページは以下の2段構成:
...
- **① 評価の傾き（Priority Donut）**: 要求が高/中/低優先度にどう分布しているか
- **② テストボリューム（Volume Bars）**: 種別別の要求件数を優先度色で横積み、全体ボリューム感を伝達
- **③ 実行結果（Execution Status）**: 実行済みなら PASS/FAIL/SKIP バー、未実行なら「⏸ 未実行」表示
